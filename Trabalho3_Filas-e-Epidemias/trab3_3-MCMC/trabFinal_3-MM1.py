...
print(f"Average Waiting Time: {mean_w:.2f}")
...
print(f"Confidence Interval: [{ciMin_w:.2f}, {ciMax_w:.2f}]\n")
...
print(f"Average number of customers in the system: {mean_c:.2f} customers")

# this block gets the confidence interval and prints it
ciMin_c, ciMax_c = confidenceInterval(samples_c, mean_c, std_c, precision_c)
print(f"Confidence Interval: [{ciMin_c:.2f}, {ciMax_c:.2f}]")

# Save the trace to a file for the MCMC code to read
with open("mm1_trace.txt", "w") as file:
